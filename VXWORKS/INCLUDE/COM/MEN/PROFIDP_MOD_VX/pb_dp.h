...
$Workfile:   pb_dp.h  $
$Author: AGromann $
$Archive:   P:\profi.vcs\pbv50\papi\inc_glb\cp5412a2\pb_dp.h_v  $
$Revision: 1.1 $
$Date: 2014/11/05 10:52:36 $
$Modtime:   12 Oct 1995 14:30:58  $
***********************  END PVCS SECTION ************************************/
...
#define DP_SL_FLAGS                 (DP_SL_ACTIVE | DP_SL_NEW_PRM)
...
typedef struct _T_DP_SLAVE_PARAM_SYS_INFO
{
  ...
  PB_INT16     diag_entries;                 /* number available DIAG messages */
...
typedef struct _T_DP_INIT_MASTER_REQ
{
  ...
  USIGN8    auto_remote_services;      /* auto remote service .res bit mask */
...
typedef struct _T_DP_GET_SLAVE_DIAG_CON
{
   ...
   USIGN8           dummy;                            /* SDU alignment byte */
   PB_INT16            diag_entries;          /* -1..DP_MAX_SLAVE_DIAG_ENTRIES */
...
#define DP_MAX_SLAVE_DIAG_ENTRIES   DP_MAX_NUMBER_SLAVES /* for each slave one entry */
#define DP_MAX_SLAVE_DIAG_LEN		DP_MAX_SLAVE_DIAG_DATA_LEN
#define DP_CLEAR_OUTPUTS			PB_TRUE /* 1 */
#define DP_AUTO_REMOTE_SERVICE		DP_AUTO_REMOTE_SERVICES /* 1 */
#define DP_CYCLC_DATA_TRANSFER		PB_TRUE /* 1 */
#define DP_CON_IND_BUF_EL           DP_MAX_NUMBER_STATIONS
#define CON_IND_BUF_ELEMENT_SIZE   (DP_MAX_TELEGRAM_LEN + sizeof( T_PROFI_SERVICE_DESCR))
#define DP_ISR_TASK_PRIO            50
/*--------------------------------------------------------------------------*/
